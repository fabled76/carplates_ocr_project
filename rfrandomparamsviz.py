...
def load_dataset(folder, allowed_classes=("0", "1", "2", "3")):
...
# Random Search for best RF parameters
param_grid = {
...
for n_estimators in n_estimators_range:
    for max_depth in max_depth_range:
        ...
        classifiers.append((n_estimators, max_depth, clf))
...
custom_cmap = ListedColormap(['blue', 'red', 'green', 'purple'])

for k, (n_estimators, max_depth, clf) in enumerate(classifiers):
    Z = clf.predict(np.c_[xx.ravel(), yy.ravel()])
    Z = Z.astype(int)
    Z = Z.reshape(xx.shape)

    plt.subplot(len(n_estimators_range), len(max_depth_range), k + 1)
    title = f"n_estimators={n_estimators}, max_depth={max_depth}"
    plt.title(title, fontsize=10)
    plt.contourf(xx, yy, Z, cmap=custom_cmap, alpha=0.3)

    # Plot training samples
    plt.scatter(X_train_2d[:, 0], X_train_2d[:, 1], c=y_train_2d.astype(int),
                cmap=custom_cmap, edgecolors="k", s=30, label='Train samples')
    # Plot test samples
    plt.scatter(X_test_2d[:, 0], X_test_2d[:, 1], c=y_test_2d.astype(int),
                cmap=custom_cmap, edgecolors="k", s=100, marker='*', label='Test samples')
    plt.xticks([])
    plt.yticks([])
...
class2_patch = mpatches.Patch(color='green', label='Class 2')
class3_patch = mpatches.Patch(color='purple', label='Class 3')
...
plt.legend(handles=[class0_patch, class1_patch, class2_patch, class3_patch, train_marker, test_marker], loc='lower right', fontsize=9)

plt.suptitle("Decision Boundaries for Different Random Forest Parameters", fontsize=14)
